  ...
  projects: [
    {
      ...
      description: "Serverless daily digest bot that emails a personalized morning summary at 8:30 AM every day — covering live weather for two cities, immigration/visa news filtered from 9 RSS and Google News feeds, and Google Calendar events — with no server required.",
      bullets: [
        "Built per-source fault isolation using a safe_fetch() wrapper across Open-Meteo, Google Calendar (OAuth2), and 9 RSS/Google News feeds — one failed source prints a warning and returns None while the rest of the digest still renders and sends.",
        "Wrote a regex-based keyword filter with word-boundary matching across 20+ terms (F-1, H-1B, OPT/CPT, SEVIS, international student policy) to extract relevant immigration articles from general news feeds, with URL deduplication across overlapping queries.",
        "Deployed serverless on GitHub Actions (cron: 30 13 * * * UTC) with encrypted repo secrets; data is rendered into an HTML email via Jinja2 and delivered over Gmail SMTP.",
      ],
